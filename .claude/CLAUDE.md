...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-24 (commit 97dabce). Confidence: 100%.
...
**Language:** rust | **Files:** 8 | **Public symbols:** 25 / 59
...
- `ui/src/views/MainView.svelte` — 1 bug fix, last fix today; 3 commits/90d
- `ui/src/components/VmTable.svelte` — 1 bug fix, last fix today; 3 commits/90d
- `crates/core/src/qemu.rs` — 3 commits/90d
- `crates/core/src/vm.rs` — 3 commits/90d
- `crates/core/src/manager.rs` — 3 commits/90d

### Code health
Three co-equal signals: defect risk 9.12/10 avg, hotspot health 6.8/10 (stable), worst `crates/core/src/store.rs` at 4.04/10 · maintainability 9.45/10 · performance risk 29 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `crates/core/src/qemu.rs` — brain method (build_qemu_args) — impact −0.6
