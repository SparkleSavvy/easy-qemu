...
<Modal title={`Snapshots — ${vm.name}`} width="580px" {onclose}>
  <div class="body">
    <div style="display:flex; gap:6px; margin-bottom:14px">
      <input type="text" bind:value={newName} placeholder="snapshot name" style="flex:1" />
      <button
        class="primary"
        disabled={busy || !newName.trim()}
        onclick={() =>
          run(() => api.snapshotCreate(vm.id, newName.trim()), `Snapshot "${newName}" created`)}
      >
        Create
      </button>
    </div>
